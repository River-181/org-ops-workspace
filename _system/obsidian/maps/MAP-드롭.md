---
title: MAP-드롭
kind: map
area: studio
status: live
created: 2026-03-26
updated: 2026-03-28
up: "[[MOC-스튜디오]]"
tags:
  - type/map
  - project/PRJ-드롭시스템
---

# MAP-드롭

> `04_studio/drops/`의 Drop 발행 인덱스.
> 상태 확인은 Base에서 하고, 운영 규칙은 이 문서에서 짧게 유지한다.

## 핵심

- [[MOC-스튜디오]]
- [[04_studio/README|04_studio/README]]
- [💬 Drops](https://www.notion.so/cnu-psy-stu/e3309c4215c74084bdd3bd92e7a37e47)

## 뷰

![[drops.base#드롭 카탈로그]]

![[drops.base#상태별]]

![[drops.base#타입별]]

## 가이드

- 발행 흐름은 `작성 -> ready -> live` 기준으로 본다.
- 전체 재고는 `드롭 카탈로그`, 운영 대기는 `상태별`, 포맷 분류는 `타입별`에서 본다.
- 개별 글 편집은 각 Drop 노트에서 하고, 이 문서는 인덱스와 운영 규칙만 유지한다.

## 상세

> [!abstract]- 그래프 링크
> - [[250926-drop-0001-huxe-ai]]
> - [[251001-drop-0002-google-gemini-spreadsheet]]
> - [[251019-drop-0003-notebooklm]]
> - [[251022-drop-0004-uxui-planning]]
> - [[251022-drop-0005-gpt-atlas-builder]]
> - [[251022-drop-0006-gpt-hallucination]]
> - [[251115-drop-0007-gpt51-group-chat]]
> - [[251201-drop-0008-gemini3-interactive]]
> - [[260101-drop-0009-gptrecap]]
> - [[260206-drop-0010-codex53-opus46]]
> - [[260217-drop-0011-claude-agent-teams]]
> - [[260305-drop-0012-openclaw-gcp-discord]]
> - [[260310-drop-0013-nugi-ot-timeful]]
> - [[260320-drop-0014-claude-pro-coupon]]
> - [[260326-drop-0015-cursor-composer2]]
> - [[260326-drop-0016-테스트]]
> - [[260401-drop-0017-4종발행]]
> - [[260403-drop-0018-claude-stats-viz]]
> - [[260414-drop-0019-report-task-switching]]
> - [[260415-drop-0020-study-3layers]]
> - [[260415-drop-0021-numbers-concept-map]]

> [!info]- 발행 규칙
> - 파일명은 `YYMMDD-drop-NNNN-제목.md`를 유지한다.
> - `drop_status`는 `inbox`, `ready`, `live`, `skipped`만 사용한다.
> - 외부 게시 전 원본은 반드시 로컬 Drop 노트에서 먼저 정리한다.

> [!note]- 운영 연결
> - Notion Drops DB: [💬 Drops](https://www.notion.so/cnu-psy-stu/e3309c4215c74084bdd3bd92e7a37e47)
> - Discord `#share`: 채널 ID `1486411209556361306`
> - CSV 백업: `04_studio/drops/drops.csv`
