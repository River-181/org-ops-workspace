...
### 3. Active Context 갱신
- `_system/agents/memory/active-context/YYMMDD-상태요약.md` 작성:
  ```yaml
  ---
  title: 상태 요약
  status: live
...
### 4. Change Log 기록
- `_system/agents/memory/change-log/YYMMDD-변경요약.md` 작성:
  - 변경된 파일 목록
  - 변경 이유
  - 영향 범위
...
- 이전 active-context 파일은 삭제하지 않는다 (이력 유지).
- change-log는 간결하게. 파일 목록 + 한 줄 이유.
- 모든 출력은 `YYMMDD-제목.md` 네이밍을 따른다.
